...
void BenchmarkKernel(const CLApp &cli, const GraphT_ &g,
  ...
  struct fault_histograms* merged = (struct fault_histograms*)malloc(sizeof(struct fault_histograms));
  struct fault_histograms* merged_min = (struct fault_histograms*)malloc(sizeof(struct fault_histograms));
  struct fault_histograms* merged_rec = (struct fault_histograms*)malloc(sizeof(struct fault_histograms));
  start_hist_recording();                  
  ...
  stop_disk_stats();
  stop_hist_recording(merged, merged_min, merged_rec);
  cleanup_counters(&perf_ctrs);
  measure_time_stats();
  printf("merged:\n");
  print_fault_histograms(merged);
  printf("merged min:\n");
  print_fault_histograms(merged_min);
  printf("merged rec:\n");
  print_fault_histograms(merged_rec);
	free(merged);
  free(merged_min);
  free(merged_rec);
