...
void measure_time_stats(double* wall_time, double* user_time, double* sys_time,
                        double* idle_time, long* major_faults, long* minor_faults){
...
struct perf_counters setup_counters(void) {
  ...
  c.fd_kernel_instr = open_counter(PERF_TYPE_HARDWARE, PERF_COUNT_HW_INSTRUCTIONS,
                                    1, 0, -1);
  /*
    The accounting for minor and major faults is done in the last lines of 
    handle_mm_fault function in mm/memory.c
  */
  c.major_faults = open_counter(PERF_TYPE_SOFTWARE, PERF_COUNT_SW_PAGE_FAULTS_MAJ,
                                    0, 0, -1);
  c.minor_faults = open_counter(PERF_TYPE_SOFTWARE, PERF_COUNT_SW_PAGE_FAULTS_MIN,
                                    0, 0, -1);
  return c;
}
...
void start_counters(struct perf_counters *c) {
  ...
  ioctl(c->major_faults, PERF_EVENT_IOC_RESET, 0);
  ioctl(c->minor_faults, PERF_EVENT_IOC_RESET, 0);
  ...
  ioctl(c->major_faults, PERF_EVENT_IOC_ENABLE, 0);
  ioctl(c->minor_faults, PERF_EVENT_IOC_ENABLE, 0);
}
...
void stop_counters(struct perf_counters *c,
                   ...
                   long long *major_faults, long long *minor_faults) {
  ...
  ioctl(c->major_faults, PERF_EVENT_IOC_DISABLE, 0);
  ioctl(c->minor_faults, PERF_EVENT_IOC_DISABLE, 0);
  ...
  ret = read(c->major_faults, major_faults, sizeof(long long));
  if (ret != (ssize_t)sizeof(long long)) {
    perror("read major_faults");
    *major_faults = -1;
  }

  ret = read(c->minor_faults, minor_faults, sizeof(long long));
  if (ret != (ssize_t)sizeof(long long)) {
    perror("read minor_faults");
    *minor_faults = -1;
  }
}
...
void cleanup_counters(struct perf_counters *c) {
  ...
  close(c->major_faults);
  close(c->minor_faults);
}
